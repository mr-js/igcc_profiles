...
if not os.path.exists(config_path):
    from config import locale, localization, profiles, timeout, keep_alive
else:
    spec = importlib.util.spec_from_file_location("config", config_path)
    config = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(config)
    locale = getattr(config, "locale", None)    
    localization = getattr(config, "localization", None)
    profiles = getattr(config, "profiles", None)
    timeout = getattr(config, "timeout", None)
    keep_alive = getattr(config, "keep_alive", None)
print(f"Config load OK: settings for locale '{locale}' selected")
